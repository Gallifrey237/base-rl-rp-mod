...
function Item:getOwner()
	if tonumber(self.m_Owner) then
		local player = DatabasePlayer.Get(tonumber(self.m_Owner))
		return player.m_Data["Name"]
	end 
	return "none"
end
	...
	if flagAmount > 0 then
		for i = 1, flagAmount, 1 do
			...
			if not ItemConditions[condition].Func(owner, self) then
				owner:sendMessage(ItemConditions[condition].Negative["DE"])
				return false
			end
		end
	end
...
function Item:checkFlagStatus()
	if not ( self:checkTemplateFlags() and self:checkConditionFlags()) then
		self:getStorage():getPlayer():sendMessage("Du erfüllst nicht alle Bedingungen!")
		return false
	end
	return true
end
